...
export default function NichePreview() {
  ...
  return (
    <div>
      ...
      {/* Формула уровня 4 — сразу видна, остальные под раскрытием */}
      <Card style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: 8 }}>
        ...
        <button
          ...
          onClick={(e) => {
            const el = document.getElementById(`formula-detail-${state.selectedNiche}`)
            if (el) {
              const isHidden = el.style.display === 'none'
              el.style.display = isHidden ? 'block' : 'none'
              e.target.textContent = isHidden ? '▲ Скрыть все уровни' : '▼ Показать все 5 уровней'
            }
          }}
        >
          ▼ Показать все 5 уровней
        </button>
        <div id={`formula-detail-${state.selectedNiche}`} style={{ display: 'none', marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
