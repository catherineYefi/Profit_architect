...
export default function NicheSelector() {
  ...
  const activeNiches = nicheList.filter(n => n.params?.length > 0)
  const comingNiches = nicheList.filter(n => !n.params?.length)

  return (
    <div>
      <SectionLabel>Шаг 1 из 7</SectionLabel>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px,1fr))', gap:10, marginBottom:28 }}>
        {activeNiches.map(n => {
          ...
          return (
            <button
              key={n.id}
              onClick={() => set({ selectedNiche:n.id, params:{}, diagnostic:null })}
              style={{
            ...
            >
              ...
              <div style={{ fontWeight:600, fontSize:14, color:isSelected?'var(--green)':'var(--text)', marginBottom:3 }}>
                {n.name}
              </div>
      ...
      {comingNiches.length > 0 && (
        <div style={{ marginTop:32, paddingTop:24, borderTop:'1px solid var(--border)' }}>
          <div style={{ fontSize:10, letterSpacing:'.15em', textTransform:'uppercase', color:'var(--text3)', marginBottom:12 }}>
            В разработке
          </div>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {comingNiches.map(n => (
              <div key={n.id} style={{ display:'flex', alignItems:'center', gap:8, background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:8, padding:'8px 12px', opacity:.45 }}>
                <div style={{ color:'var(--text3)' }}>{NICHE_ICONS[n.id]}</div>
                <span style={{ fontSize:12, color:'var(--text3)' }}>{n.name}</span>
              </div>
            ))}
          </div>
        </div>
