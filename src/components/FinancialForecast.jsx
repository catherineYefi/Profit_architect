...
function calcPctGap(current, target) {
  ...
  const pct = Math.round((target - current) / current * 100)
  return pct
}

// Целевая прибыль: от целевой выручки и целевой маржи минус косты × 1.1
function calcTargetProfit(currentRevenue, targetMargin, fixedCosts) {
  const tRevenue = Math.round(currentRevenue * 1.4)
  const tCosts   = Math.round(fixedCosts * 1.1)
  // Для целевой модели используем targetMargin как % чистой прибыли от выручки
  // Это согласованно: targetMargin = NICHE_TARGET_MARGINS = рентабельность
  return Math.max(0, tRevenue * targetMargin / 100)
}
...
export default function FinancialForecast() {
  ...
  // ── Текущие показатели (правильный расчёт через getNetProfit) ──
  ...
  const currentProfit  = getNetProfit(state.params, nicheId)   // ← правильно
  ...
  // Целевая прибыль: рентабельность targetMargin% от targetRevenue
  const targetProfit  = Math.max(0, targetRevenue * targetMargin / 100)
  const targetRent    = targetMargin // по определению = targetMargin при нашей модели
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom:14 }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 80px 80px 1fr', fontSize:10, letterSpacing:'.08em', textTransform:'uppercase', color:'var(--text3)', paddingBottom:8, marginBottom:4, borderBottom:'1px solid var(--border)', gap:8 }}>
          <div/>
        ...
        {pnlRows.map((r, i) => (
          <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 80px 80px 1fr', gap:8, padding:'7px 0', borderBottom:i<pnlRows.length-1?'1px solid var(--border)':'none', fontWeight:r.bold?600:400 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:5, justifyContent:'center' }}>
              ...
              {r.gapPct !== null && r.gapPct !== undefined && (
                <span style={{ fontSize:10, padding:'1px 5px', borderRadius:3, fontWeight:600,
                  background: r.color==='var(--green)'?'rgba(45,191,138,.12)':r.color==='var(--amber)'?'rgba(245,158,42,.1)':'rgba(139,124,246,.12)',
                  color: r.color }}>
                  {r.gapPct > 0 ? '+' : ''}{r.gapPct}%
                </span>
              )}
      ...
      <Card style={{ marginBottom:14 }}>
        ...
        <Slider label="Дивиденды клиенту" value={dividendClient} min={0} max={80} step={5}
          onChange={v => set({ dividendClient: v })} color="var(--amber)" />
        <Slider label="Дивиденды фонду"   value={dividendFund}   min={10} max={25} step={5}
          onChange={v => set({ dividendFund: v })}   color="var(--purple)" />
